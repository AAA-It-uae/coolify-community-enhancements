<div data-vitals-live-fix="20260916" data-vitals-refreshed-at="{{ now()->getTimestamp() }}" wire:poll.5s.visible="refreshVitals"
    ...
    title="Live server resources · red = used, green = free/available · refreshes every 5 seconds while visible">
<style>
    /* Progressive disclosure keeps the fixed top bar collision-free on narrower screens. */
    @media (max-width: 1999px) {
        .local-vitals-load-extra { display: none !important; }
    }
    ...
    <div class="local-vitals-load flex shrink-0 items-center gap-1.5 rounded-lg border px-2 py-1"
        ...
        <span class="local-vitals-load-extra inline-flex items-center gap-1.5">
        <span class="text-neutral-300 dark:text-white/15">·</span>
